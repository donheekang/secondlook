...
GEMINI_MODEL = os.getenv("GEMINI_MODEL", "gemini-1.5-flash").strip()  # you can set "gemini-3.0-flash" etc
...
REDIS_URL = os.getenv("REDIS_URL", "").strip()  # e.g. redis://:pass@host:port/0
...
ALLOWED_ORIGINS = os.getenv("ALLOWED_ORIGINS", "")  # "" or comma-separated list
...
class AnalyzeRequest(BaseModel):
    ...
    asset: Optional[str] = Field(default=None, description="US|KR|COIN (optional; server can infer)")
...
def now_iso() -> str:
    # KST ISO8601 to match your UI feeling (still ISO format)
    # If you want strict UTC, switch to datetime.now(timezone.utc).
    kst = timezone.utc  # keep UTC by default for consistency
...
def score_sentence(sentence: str, asset: str) -> Tuple[float, List[str], str]:
    ...
    bump("D", KW_D, 1.0)
    if asset == "COIN":
        bump("E", KW_E_coin, 1.2)
    else:
        bump("E", KW_E_stock, 1.0)

    # boost base score by best slot score
    best_slot = max(slot_scores.items(), key=lambda x: x[1])[0]
    best_val = slot_scores[best_slot]
    score += best_val * 1.2
...
def tier_for_domain(asset: str, domain: str) -> str:
    ...
    primary_coin = {"coinmarketcap.com", "coingecko.com"}  # data sites (reputable). official docs vary
    ...
    if asset == "KR" and d in primary_kr:
        return "primary"
    # coin official docs differ by project; treat common data sites as reputable
...
def dedup_and_rank_sources(asset: str, raw_hits: List[Dict[str, Any]], limit: int = MAX_SOURCES) -> List[Dict[str, Any]]:
    # Dedup by URL + domain cap
    by_domain_count: Dict[str, int] = {}
    seen_url = set()
    out = []

    # simple ranking: primary/reputable domains first
    ...
    for h in ranked:
        ...
        out.append({
            ...
            "url": url,
            "publisher": dom,
            "published_at": "",  # MVP: unknown unless provider returns it
            "tier": tier_for_domain(asset, dom)
        })
        if len(out) >= limit:
            break
...
# =========================================================
# Fetch & Excerpt extraction (rule-based)
# =========================================================
...
async def fetch_text(url: str, sem: asyncio.Semaphore) -> Tuple[str, Optional[str]]:
    async with sem:
        async with httpx.AsyncClient(
            ...
            # If BeautifulSoup missing, return raw text (poor but works)
...
async def build_excerpts(asset: str, sources: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # add stable ids s1..sn
    for i, s in enumerate(sources, start=1):
        s["id"] = f"s{i}"

    tasks = [process_source(s) for s in sources]
    all_chunks = await asyncio.gather(*tasks, return_exceptions=True)
    ...
    # enforce total limit and stable ordering:
    # sort by whether excerpt has digits, then by source_id
...
def filter_sources_to_allowed(obj: Dict[str, Any], allowed_sources: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # map url -> title (prefer server title)
...
def ensure_three_questions(obj: Dict[str, Any], field: str, conviction: str) -> None:
    ...
    if len(qs) > 3:
        qs = qs[:3]
    while len(qs) < 3:
        # fallback – must reference conviction
        if len(qs) == 0:
...
async def retrieve_context(asset: str, ticker: str, company: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    # If search not configured, keep empty
    if not sources:
        return [], []
...
async def run_short(asset: str, ticker: str, company: str, conviction: str, force_refresh: bool) -> Dict[str, Any]:
    ...
    # If top sources empty, provide at least blindspot sources (already filtered)
...
async def analyze(req: AnalyzeRequest):
    # This is your "Short Report" endpoint for the app.
    ...
    out = await run_short(asset, ticker, company, conviction_norm, req.force_refresh)
    # keep backward-compatible field name "questions" already present
    # keep optional "sources"
    return out
...
async def deep_report(req: DeepReportRequest):
    ...
    out = await run_deep(asset, ticker, company, conviction_norm, req.force_refresh)
    return out

# Optional: short-report alias
